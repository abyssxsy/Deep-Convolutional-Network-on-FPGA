...
def gen_softmax_data(filename='data.bin',ni=500,no=10,batch_size=384):
    ...
    fmaxz = maxz
    fexpz = [expz[i2][i3] for i2 in xrange(no) for i3 in xrange(batch_size)]
    fsumexpz = sumexpz
    ...
    with open(filename,'w') as ouf:
        ...
        cPickle.dump(fw,ouf)
        cPickle.dump(fz,ouf)
        cPickle.dump(fmaxz,ouf)
        cPickle.dump(fexpz,ouf)
        cPickle.dump(fsumexpz,ouf)
...
def gen_all_data():
    ...
    gen_softmax_data(filename=dns[4]+'data3.bin',ni=10,no=10,batch_size=384)
    gen_softmax_data(filename=dns[4]+'data4.bin',ni=50,no=10,batch_size=384)
    gen_softmax_data(filename=dns[4]+'data5.bin',ni=200,no=10,batch_size=384)
    gen_softmax_data(filename=dns[4]+'data6.bin',ni=500,no=10,batch_size=384)
    gen_softmax_data(filename=dns[4]+'data7.bin',ni=500,no=10,batch_size=384)
